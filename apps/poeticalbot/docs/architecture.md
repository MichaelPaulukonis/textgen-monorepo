...
- **Entry Point**: `src/cli.js` with Commander.js CLI
- **Environment**: Node.js process with dotenv configuration
- **Output**: Console logging or Tumblr posting

#### Production (AWS Lambda)

- **Entry Point**: `src/lambda-handler.js` (same `src/` tree as the CLI, packaged by `build-lambda.sh`)
- **Trigger**: CloudWatch/EventBridge scheduled event
